...
int p_objects(ezpp_t ez, slice_t* line) {
    ...
    if (ez->mode == MODE_TAIKO) {
        int* sound_type = (int*)m_alloc(ez, sizeof(int));
        if (!sound_type) {
            return ERR_OOM;
        }
     ...
     */
        ...
        if (ez->mode == MODE_TAIKO && ne > 8 && slice_len(&e[8]) > 0) {
            ...
            o->sound_types = (int*)m_alloc(ez, sizeof(int) * nodes);
            if (!o->sound_types) {
                return ERR_OOM;
            }
...
#define s(x) ez->x = ez->x ? ez->x : (char*)"(null)"
    s(title);
    s(title_unicode);
    s(artist);
    s(artist_unicode);
...
ezpp_t ezpp_new(void) {
    ezpp_t ez = (ezpp_t)calloc(sizeof(struct ezpp), 1);
    if (ez) {
        ez->mode = MODE_STD;
        ez->mods = MODS_NOMOD;
        ez->combo = -1;
...
int ezpp_data_dup(ezpp_t ez, char* data, int data_size) {
    free_owned_map(ez);
    ez->flags |= OWNS_MAP_BIT;
    ez->data = (char*)memclone(data, data_size);
    ez->data_size = data_size;
    return calc(ez);
}
